...
public class VideoInviteActivity extends AppCompatActivity {
     ...
     */
    public static final String TWILIO_SDK_STARTER_SERVER_URL = "https://shamsaha.com/app/twilio/";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Toast.makeText(this, "caseId:"+caseId, Toast.LENGTH_SHORT).show();
        ...
        if (!checkPermissionForCameraAndMicrophone()) {
        ...
        } else if (intent1 != null && intent1.getAction() == ACTION_REGISTRATION) {
            handleRegistration(intent1);
            Toast.makeText(this, "1", Toast.LENGTH_SHORT).show();

        } else if (intent1 != null && intent1.getAction() == ACTION_VIDEO_NOTIFICATION) {
            /*
            ...
            cachedVideoNotificationIntent = intent1;
            register();
        } else {
...
//            statusTextView.setText(R.string.registered);
    ...
    private void sendRegistrationSuccess(Notification notification) {
        Intent intent = new Intent(VideoInviteActivity.ACTION_VIDEO_NOTIFICATION);
        intent.putExtra(VideoInviteActivity.REGISTRATION_IDENTITY, notification.body);
        intent.putExtra(VideoInviteActivity.REGISTRATION_TOKEN, notification.title);
...
//            statusTextView.setText("Connected to " + room.getName());
    ...
    void notify(final String roomName) {
        ...
        if (SharedPreferencesUtils.getIsLoggedIn()){
            ...
            identity = SharedPreferencesUtils.getIdentityToInviteForVideoCall();
            //identity ="SV000071";
        }
        TwilioSDKStarterAPI.videoNotify(caseId,identity).enqueue(new Callback<Void>() {
            @Override
            public void onResponse(@NonNull Call<Void> call, @NonNull Response<Void> response) {
                if (response.isSuccessful()){
                    Log.i(TAG, "onResponse: video notification sent");
                }else{
                    Log.i(TAG, "onResponse: Something's wrong");
                }
...
//        connectActionFab.setOnClickListener(connectActionClickListener());
        ...
        if (caseId.isEmpty()) {
            roomName = ChanModel.getInstance().getChannel().getFriendlyName();
        } else {
            roomName = caseId;
        }
